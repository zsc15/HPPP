...
function [im_out, psnr_out] = RunSCFP(y, ForwardFunc, BackwardFunc,...
...
for k = 1:1:outer_iters   
    Ax = ForwardFunc(x_est);
    P_Ax = Projection_On_L2_Ball(Ax, y, input_sigma);
    r = P_Ax - Ax;
    norm_r = norm(r,'fro')^2;
    A_star_r = BackwardFunc(r);
    norm_A_star_r = norm(A_star_r,'fro')^2;
    if norm(Ax-y, 2)>input_sigma
        Lx = x_est+ 0.5*norm_r/norm_A_star_r*A_star_r;
    else
        Lx = x_est;
    end
    % denoise
